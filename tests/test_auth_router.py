...
mock_email_service.send_email.return_value = {"status": "success", "message": "Email sent"}
...
def mock_redis():
    ...
    with patch('fastapi_otp_auth.auth_router.get_redis_client') as mock_redis_dep:
    ...
    mock_redis_instance = AsyncMock()
    app.dependency_overrides['fastapi_otp_auth.auth_router.get_redis_client'] = lambda: mock_redis_instance 
...
def test_request_otp_email_failure(mock_redis, test_email):
    ...
    # Setup mock to fail
    mock_email_service.send_email.return_value = {"status": "error", "message": "SMTP Error"}
...
def test_logout_success(mock_redis, test_email, test_otp):
    ...
    assert 'refresh_token=""' in response.headers["set-cookie"] or "refresh_token=;" in response.headers["set-cookie"]
